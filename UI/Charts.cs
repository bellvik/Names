...
internal static class Charts
{
	public static void ShowHistogram(HistogramData stats, string title)
	{
		...
		{
			LegendPlacement = LegendPlacement.Outside, LegendPosition = LegendPosition.RightBottom,
	...
	public static void ShowLineSeries(LineData stats, string title)
	{
        var model = new PlotModel { Title = stats.Title };
		...
		foreach (var e in stats.Count)
		{
			line.Points.Add(new DataPoint(e.Key, e.Value));
		}
